...
function sendOTP($otp, $phone) {
    ...
    if($stmt) {
    	//send otp.   
    	$curl = curl_init();
        $message = "<#>OTP to activate your CorsTech account is: $otp. Do not share this with anyone by any means.";
        curl_setopt_array($curl, array(
          CURLOPT_URL => "https://www.fast2sms.com/dev/bulkV2?authorization=YOUR_API_KEY_HERE&message=".urlencode($message)."&language=english&route=q&numbers=".urlencode($phone),
...
function updateProfile() {
    ...
    if(empty($_POST['name'])) {
        array_push($formErrors, 'Please enter your name');
    }

    if(empty($_POST['email'])) {
        array_push($formErrors, 'Please enter your email');
    }

    if(empty($_POST['age'])) {
        array_push($formErrors, 'Please enter your age');
    }
    ...
    if(empty($formErrors)) {
        ...
        $stmt->bind_param('sssi', $name, $email, $age, $user_id);
        $stmt->execute();

        if($stmt) {
            echo "<script>alert('Your profile has been updated!')</script>";
        } else {
            echo "<script>alert('An error occurred while updating profile, please try again!')</script>";
        }
    }
}
...
function userPayments() {
    ...
    $rows = array();
    $stmt = $conn->prepare("SELECT bookings.amount, bookings.payment_status, bookings.payment_id, bookings.category, bookings.added_on, bookings.invoice FROM bookings JOIN users ON bookings.user_id = users.id WHERE users.id = ? ORDER BY bookings.id DESC");
    $stmt->bind_param('i', $user_id);
    $stmt->execute();
