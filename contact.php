<?php
require('categories/assets/includes/db.php');
...
if(isset($_POST['name']) && isset($_POST['email']) && $_POST['message']) {
    ...
    if(updateContactQuery($name, $email, $message)) {
        $test = 12;
    } else {
        ...
        $mail->setFrom('YOUR_EMAIL_HERE', 'No Reply | CorsTech');
        $mail->addAddress('SUPPORT_TEAM_EMAIL_HERE');  
...
function updateContactQuery($name, $email, $message) {
    global $conn;
    $stmt = $conn->prepare("INSERT INTO `contact_queries`(`name`, `email`, `message`, `status`) VALUES(?, ?, ?, 'unattended')");
...
<head>
    ...
    <title>CorsTech | Contact Us</title>
</head>
<body>
   <nav class="navbar bg-white shadow-lg">
      ...
      <div class="hidden mobile-menu">
        <ul>
          <li class="active">
            <a href="./" class="block text-sm px-2 py-4 text-white bg-green-500 font-semibold">Home</a>
          </li>
          <li>
